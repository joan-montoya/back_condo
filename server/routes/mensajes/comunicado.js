/*jshint esversion: 9*/
const ComunicadoModel = require('../../models/comunicado.model');
...
app.get('/', async(req, res) => {
    try {
        ...
        const mensaje = await ComunicadoModel.aggregate([   
            {
                $lookup: {
...
app.post('/', async(req, res) => {
    ...
    try {
        const men = new ComunicadoModel(req.body);
        ...
        const usuarioEncontrado = await ComunicadoModel.findOne({ strCorreo: { $regex: `^${men.strCorreo}$`, $options: 'i' } });
