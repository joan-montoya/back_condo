...
app.get('/', async(req, res) => {
    try {
        ...
        const cuenta = await PagosModel.aggregate([   
            {
                $lookup: {
        ...
        if (cuenta.length <= 0) {
            res.status(404).send({
                estatus: '404',
                err: true,
                msg: 'No se encontraron pagos en la base de datos.',
                cont: {
                    cuenta
                } 
            });
...
app.post('/', async(req, res) => {
    ...
    try {
        const cuenta = new PagosModel(req.body);
        ...
        const CuentaEncontrada = await PagosModel.findOne({ strNumTarjeta: { $regex: `^${cuenta.strNumTarjeta}$`, $options: 'i' } });
...
app.put('/', async(req, res) => {
    try {
        ...
        const cuentaEncontrada = await PagosModel.findById(idCuenta);
        ...
        const newCuenta = new PagosModel(req.body);
        ...
        const cuentaActualizada = await PagosModel.findByIdAndUpdate(idCuenta, { $set: newCuenta }, { new: true });
